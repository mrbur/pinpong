...
TArray<FString> UDLCLoader::LoadDLC()
{
	...
	// Get DLC folder
	FString PathToDLCFolder =
		FPaths::ConvertRelativePathToFull(FPaths::ProjectDir()) + "DLC";
	UE_LOG(LogTemp, Warning, TEXT(" PATH: %s "), *PathToDLCFolder);
	IFileManager& FileManager = IFileManager::Get();
	if (!FPaths::DirectoryExists(PathToDLCFolder))
	{
		UE_LOG(LogTemp, Warning, TEXT("DLC Directory %s not found"),
			*PathToDLCFolder);
	...
	FileManager.FindFilesRecursive(PAKFiles, *PathToDLCFolder,
		*DLCExtension, true, false);
...
{
	UE_LOG(LogTemp, Warning, TEXT("ReadPakFile: Mount pak file : %s"),
		*PakFileName);
	FPakPlatformFile* PakPlatformFile;
	{
		...
		else
		{
			PakPlatformFile = new FPakPlatformFile;
				...
				if
					(!PakPlatformFile->Initialize(&FPlatformFileManager::Get().GetPlatformFile(),
						TEXT("")))
				{
					UE_LOG(LogTemp, Error, TEXT("FPakPlatformFile failedto initialize"));
						return false;
				}
			FPlatformFileManager::Get().SetPlatformFile(*PakPlatformFile);
		}
	}
	FString PakFilePathFull =
		FPaths::ConvertRelativePathToFull(PakFileName);
	...
	for (int32 k = 0; k < FileList.Num(); k++)
	{
		...
		if (AssetName.Contains("MapInfo") && AssetName.Contains(".uasset"))
		{
			...
			FString NewAssetName = FString("/") + packName + "/" +
				FileName + TEXT(".") + FileName + "_C";
			UE_LOG(LogTemp, Log, TEXT("Loading Asset %s ..."),
				*NewAssetName);
			UObject * objectToLoad = nullptr;
			FStringAssetReference ref =
				FStringAssetReference(NewAssetName);
...

			{
				UBlueprintGeneratedClass* generatedClass =
					Cast<UBlueprintGeneratedClass>(objectToLoad);
				UMapInfoAsset* mapInfoObject =
					Cast<UMapInfoAsset>(generatedClass->GetDefaultObject());
				// Take default object by "info" class
				if (mapInfoObject)
				{
					_mapsInfo.Add(mapInfoObject->_mapInfo);
					UE_LOG(LogTemp, Warning, TEXT("Add data %s"),
						*mapInfoObject->GetFullName());
				}
				else
				{
					UE_LOG(LogTemp, Log, TEXT("File %s loadingerror!"), *FileList[k]);
				}
			}
		}
	}
...
FPakPlatformFile * UDLCLoader::GetDLCFile()
{
	if (!DLCFile)
	{
		...
		{
			DLCFile =
				static_cast<FPakPlatformFile*>(CurrentPlatformFile);
		}
		else
		{
			...
			{
				UE_LOG(LogTemp, Error, TEXT("DLCFile initializationerror!"));
			}
		}
	}
	ensure(DLCFile != nullptr);
...
void UDLCLoader::RegisterMountPoint(const FString& RootPath, const FString&
	ContentPath)
{
...
{
	const FString Name = Filename + TEXT(".") +
		FPackageName::GetShortName(Filename) + TEXT("_C");
	...
	if (assetManager->GetAssetRegistry().GetAllAssets(RegisteryData))
	{
		for (FAssetData& data : RegisteryData)
		{
			for (FString classToLoad : classesToLoad)
			{
				...
				{
					UObject* objectToLoad = nullptr;
					auto generatedClassPath =
						data.TagsAndValues.FindTag(FName("GeneratedClass"));
					if (generatedClassPath.IsSet())
					{
						...
						const FString ClassName =
							FPackageName::ObjectPathToObjectName(ClassObjectPath);
						objectToLoad =
							FStringAssetReference(ClassObjectPath).TryLoad();
					}
					if (objectToLoad)
					{
...
TArray<FString> UDLCLoader::GetClassesToLoad()
{
	...
	{
		/*Trim class name to get ClasName instead of
		ClassName.ClassName_C*/
